...
## How to update IDEs:
 - Run `./updater/main.py`, this will update binary and source IDEs. After this you can commit them
   ...
   - The IDEs have `passthru.updateScript` set up to run `./updater/main.py`. The script then uses the `UPDATE_NIX_*`
...
## How to add an IDE:
 ...
 - Add an entry to the URL templates in `updater/url-templates.json`
